# -*- coding: utf-8 -*-
"""Data Preprocessing & Model Comparison.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IuSog1r4m5ct5x6I_yuw9wTCiDO6EQJL
"""
...
best_rf_tuned.fit(X_tu_resampled, y_tu_resampled)

# Make predictions on the original test set
y_pred_best_rf_tuned = best_rf_tuned.predict(x_tu_test)
y_prob_best_rf_tuned = best_rf_tuned.predict_proba(x_tu_test)[:,1]
# Evaluate the model
train_accuracy_rf_tuned = accuracy_score(y_tu_resampled, best_rf_tuned.predict(X_tu_resampled))
test_accuracy_rf_tuned = accuracy_score(y_tu_test, y_pred_best_rf_tuned)
...
#threshold = 0.40
y_prob_best_rf_tuned1 = best_rf_tuned.predict_proba(x_tu_test)[:,1]

for threshold in [0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60]:

    y_pred_best_rf_tuned1 = (y_prob_best_rf_tuned1 >= threshold).astype(int)

    tn, fp, fn, tp = confusion_matrix(y_tu_test, y_pred_best_rf_tuned1).ravel()
...
best_rf_tuned_finel.fit(X_tu_resampled, y_tu_resampled)

y_prob_best_rf_tuned1 =  best_rf_tuned_finel.predict_proba(x_tu_test)[:,1]

threshold_ftur_slc1 = 0.2

y_pred_best_rf_tuned1 = (y_prob_best_rf_tuned1 >= threshold_ftur_slc1).astype(int)

print(classification_report(y_tu_test, y_pred_best_rf_tuned1))

train_pred_rf_tu =  best_rf_tuned_finel.predict(X_tu_resampled)
test_pred_rf_tu =  best_rf_tuned_finel.predict(x_tu_test)

train_accuracy_rf_tu = accuracy_score(y_tu_resampled, train_pred_rf_tu)
test_accuracy_rf_tu = accuracy_score(y_tu_test,test_pred_rf_tu)
...
print(confusion_matrix(y_tu_test,y_pred_best_rf_tuned1))
...
rf_balanced_finel = BalancedRandomForestClassifier(n_estimators=100, random_state=42, class_weight='balanced')
rf_balanced_finel.fit(X_resampled4, y_resampled4)

Y_prob_rf_balanced3 =  rf_balanced_finel.predict_proba(x_brf_test)[:,1]
...
train_pred_rf_tu =  rf_balanced_finel.predict(X_resampled4)
test_pred_rf_tu = rf_balanced_finel.predict(x_brf_test)
...
pipeline = Pipeline([
    ('rf_balanced_finel', BalancedRandomForestClassifier())
])
